...
async function get_is_running(): Promise<boolean> {
  const data = await fetch(
    process.env.NEXT_PUBLIC_API_URL + "/api/is_running",
    {
      method: "POST",
      headers: {
...
export function Console() {
  ...
  const sendMessages = useRef<string[]>([]);
  const already_sent_one = useRef(false);

  const [socketUrl] = useState<string | null>(WS_URL);
  ...
  const handleClickSendMessage = async () => {
    if (cmd.current && cmd.current.value) {
  ...
  const keyUpHandler = (event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === "Enter") {
      ...
      if (sendMessages.current.length > 0 && cmd.current) {
        cmd.current.value =
          sendMessages.current[sendMessages.current.length - 1];
      }
    }
  };
    ...
    <div className="bg-gray-900 text-gray-100 w-full relative h-[39rem] rounded-sm border-white border pt-3">
      ...
      <div className="px-4 overflow-y-scroll break-words h-[35rem] flex flex-col-reverse">
        ...
        {messageHistory
          .slice()
          .reverse()
          .map((message: MessageEvent, idx: number) => (
            <span
              key={idx}
              dangerouslySetInnerHTML={
                message
                  ? { __html: convert.toHtml(message.data) }
                  : { __html: "" }
              }
            />
          ))}
      ...
      <div className="flex bottom-0 left-0 bg-gray-800 px-4 outline-none absolute border-t h-12 border-white w-full text-gray-100">
        <span className="h-full text-xl pt-[0.475rem] pr-2 text-gray-400">
          {" "}
          {">"}{" "}
        </span>
        <input
          className="h-full bg-transparent outline-none w-full"
